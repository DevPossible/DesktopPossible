...
installer. DesktopPossible includes an automated migration engine that
...
`DesktopPossible-<version>-win-x64.zip` from the
[releases page](https://github.com/DevPossible/DesktopPossible/releases)
and unzip it anywhere you like (for example `C:\Tools\DesktopPossible`).
...
Paste both directly into your **new** DesktopPossible folder. If
prompted, choose to overwrite/replace any existing files.

**Step 4: Launch and let it upgrade.** Run `DesktopPossible.exe`. On
